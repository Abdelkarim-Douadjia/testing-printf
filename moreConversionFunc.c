 ...
 * a pointer variable in hexadecimal
...
int printfPointer(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
	...
	void *addrs = va_arg(listType, void *);
	...
	if (flags & PLUS)
		...
		ch = ' ', len++;
	ind++;
	return (writePointer(buffer, ind, len, width, flags,
				padding, ch, paddbegin));
}

/**
 * printfUnPrintable - function that prints the ASCII codes in
 * hexa of unprintable characters
...
int printfUnPrintable(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
	int i = 0, offset = 0;
...
int printfReverse(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
	char *str;
	int i, count = 0;
...
int printfRot13String(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
	char temp;
	char *str;
