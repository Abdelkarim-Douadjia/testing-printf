...
int printfBinary(va_list listType, char buffer[], int flags, int width,
 ...
 * in octal notation, handling various formatting options
 * such as width, precision, and flags.
...
int printfOctal(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
	int i = BUFFER_SIZE - 2;
	...
	if (flags & HASH && temp != 0)
		buffer[i--] = '0';
	i++;
	return (writeUnsignd(0, i, buffer, flags, width,
				precision, size));
}

/**
 * printfHexadecimal - function that prints an unsigned number
 * in hexadecimal notation
 * Description: this function prints an unsigned number in
...
int printfHexadecimal(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
return (printfHexa(listType, "0123456789abcdef", buffer,
				flags, 'x', width, precision, size));
}


/**
 * printfHexaUpper - function that prints an unsigned number in
...
int printfHexaUpper(va_list listType, char buffer[], int flags, int width,
		int precision, int size)
{
return (printfHexa(listType, "0123456789ABCDEF", buffer, flags, 'X',
			width, precision, size));
}


/**
 * printHexa - function that prints an hexadecimal number
 * in lowercase or uppercase
 * Descrption: this function is a generic function
 * for printing an unsigned number
...
int printfHexa(va_list listType, char mapto[], char buffer[], int flags,
